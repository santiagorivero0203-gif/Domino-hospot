...
export default function OpponentHands({ players, localPlayerId, currentTurnIndex }: OpponentHandsProps) {
  ...
  return (
    <div className="absolute top-6 left-0 right-0 z-20 pointer-events-none flex justify-center gap-2 sm:gap-12 px-2 sm:px-4">
      {opponents.map((opponent) => {
        const isCurrentTurn = players[currentTurnIndex]?.id === opponent.id;
        
        return (
          <div key={opponent.id} className="flex flex-col items-center gap-1 sm:gap-2">
            {/* Nombre e indicador de turno del oponente */}
            <div 
              className={`text-[8px] sm:text-[10px] font-bold uppercase tracking-widest px-2 sm:px-3 py-1 rounded-full backdrop-blur-md transition-all duration-300 ${
                isCurrentTurn 
            ...
            <motion.div layout className="flex gap-[1px] sm:gap-[2px]">
              <AnimatePresence>
                {opponent.hand.map((tile) => (
                  <motion.div
                    ...
                    className="w-[10px] h-[16px] sm:w-[22px] sm:h-[34px] rounded-[2px] sm:rounded-[3px] border border-black/80 shadow-md"
                    style={{
